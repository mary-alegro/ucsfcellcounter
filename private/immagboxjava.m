...
cb_listeners = {};
    ...
    function setScrollpanel(scrollpanel_api)
        ...
        if ~isempty(scrollpanel_api)
            sp_api = scrollpanel_api;
        else
            % The @IPTPanZoom class used in implay requires the ability to
            % disassociate a current immagboxjava instance from a scrollpanel
            % in the VisualChanged workflow in which scrollpanels may be
            % destroyed and recreated during the lifetime of an immagboxjava
            % instance. We disassociate a magcombo box instance by
            % specifying empty in the call to setScrollpanel.
            cb_listeners = {}; %#ok<*SETNU>
            return
        end
    ...
    function updateMag(varargin)
        ...
        if (index==0) % Fit-To-Window
           ...
           if (~isempty(oldMag) && ~isempty(newMag))
               if images.internal.magPercentsDiffer(newMag, oldMag)
                   setMag(newMag)
               end
           end           
        end

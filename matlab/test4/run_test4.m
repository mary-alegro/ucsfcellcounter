%CELLC
%Compares our ground truth with masks created CellC
...
test4_mask_G = strcat(root_dir,'toprocess/tests_paper/test4/channels/G_result/');
test4_mask_R = strcat(root_dir,'toprocess/tests_paper/test4/channels/R_result/');
...
for i=1:nFiles
    ...
    mask_G = imread(strcat(test4_mask_G,'green_',file_name2,'_totcount_result.tif')); %load green mask
    mask_R = imread(strcat(test4_mask_R,'red_',file_name2,'_totcount_result.tif')); %load red mask
    mask_G = logical(mask_G);
    mask_R = logical(mask_R);
    mask = (mask_G | mask_R);

...
function [mask,R2,G2,B2,centersX,centersY,types] = test_svmpar(R,G,B,origR,origC,doResize)

S = load('model_svm2.mat');
model = S.model;
S = load('mins_svm2.mat');
mins = S.mins;
S = load('ranges_svm2.mat');
ranges = S.ranges;
...
%%% refine segmentation using SVM
    ...
    parfor i=1:nData
        ...
        D = data(i,:);
        L = 0;
        [C, precision, probs] = svmpredict(L, D, model);
        classes(i) = C;
    end

...
% PTS: points set to be filtered in INDICE format
% MIN_DIST: minimum edge size
%
% CLEANR: clean points ROW
% CLEANC: clean points COL
% CLEAN_POINTS: thresholded points in indices format
%

[r c N] = size(img);

% [R1,C1] = ind2sub([r c],pts1);
% [R2,C2] = ind2sub([r c],pts2);
% R = [R1; R2];
% C = [C1; C2];
[R,C] = ind2sub([r c],pts);
...
removedR = R(removePts);
removedC = C(removePts);

...
%    that allows you to change the colormap of the target figure by
%    selecting a colormap from a list of MATLAB colormap functions,
...
[hClientFig,hImage] = parseInputs(varargin{:});
...
hCMapFig = figure('Toolbar','none',...
    ...
    'Name',createFigureName('Choose Colormap',hClientFig),...
    'WindowStyle','normal',...
    'Resize','off');
...
originalColormap = get(hClientFig,'Colormap');
...
% Add listeners in order to listen to colormap changes
cmapListener = iptui.iptaddlistener(hClientFig,'Colormap','PostSet',...
    @(varargin) changeFcn());
setappdata(hCMapFig,'colormapListener',cmapListener);
clear cmapListener;
reactToImageChangesInFig(hImage,hCMapFig,@deleteFcn,@refreshFcn);
registerModularToolWithManager(hCMapFig,hImage);
...
lastCustom = [];
    ...
    function deleteFcn(varargin)
        ...
        if ishghandle(hCMapFig)
            ...
            if userCanceled
                if ~isempty(originalColormap)
                    setFigColormap(hClientFig,originalColormap);
                end
        ...
        if ishghandle(hClientFig) && ...
                isappdata(hClientFig,'imcolormaptoolFig')
    ...
    function changeFcn(varargin)
        % This function gets called only if colormap is changed from
        % outside this tool.
        
        % get new map
        cmap = get(hClientFig,'Colormap');
    ...
    function hPanel = createCMapFcnPanel
        ...
        % Check if colormap of hClientFig is one of the predefined
        % colormaps
        isFound = false;
        for i = 1:length(cmapStr)
            if isequal(cmapStr{i,2}(256),get(hClientFig,'Colormap'))
    ...
    function status = getColormap
            ...
            if isFunctionList
                ...
                if ~isempty(strfind(selectedStr,['<' customStr '>']))
                    if ~isempty(strfind(selectedStr,[' (' originalStr ')']))
                        cmapOut = originalColormap;
                    else
                        cmapOut = lastCustom;
                    end
                    selectedStr = '';
                    if ~isempty(cmapOut)
                        status = setFigColormap(hClientFig, cmapOut);
                    end
        ...
        if ~isdeployed
            ...
            try
                ...
                stat = setFigColormap(hClientFig,cmapOut);
            catch ME
            ...
            % If the colormap "cmapOut" is invalid, do not apply it to the
            % figure
            if ~stat, return, end
        ...
        % If no item is selected, then don't update figure's colormap
        if ~isempty(cmapOut)
            status = setFigColormap(hClientFig, cmapOut);
        end
    ...
    %--------------------------------
    function stat = setFigColormap(hFig,map)
        ...
        try
            ...
            if strcmp(ME.identifier,'images:setFigColormap:badMapValues')
                error_str = ME.message;
            elseif strcmp(ME.identifier,'images:setFigColormap:badMapMatrix')
                error_str = ME.message;
            else
...
%------------------------------------
function [hClientFig,hImage] = parseInputs(varargin)
...
% get client figure
if nargin == 1
    % Check if supplied hClientFig is a valid figure handle
    hClientFig = varargin{1};
    iptcheckhandle(hClientFig,{'figure'},mfilename,'HCLIENT',1);
else
    hClientFig = get(0,'CurrentFigure');
end

% Check if there is at least one grayscale or indexed image
imHandle = findall(hClientFig,'type','image');
hImage = [];

classdef imcropRectButtonDown < iptui.imcropRect
    ...
    methods
        ...
        function completeCrop(obj)
            ...
            % parent figure.
            dist_lines = findobj(h_fig,'tag','distance label');
            ...
            if ~isempty(new_cdata)
                % cache old clim
                oldCLim = get(h_ax,'CLim');
                
                hsp = findall(h_fig,'tag','imscrollpanel');
                hsp_api = iptgetapi(hsp);
                
                % account for indexed images
                is_indexed_image = strcmpi(get(h_im,'CDataMapping'),'direct');
